...
R_Solution::R_Solution(R_Repository repository, int height, int width, bool rotatable)
	: _repository(repository), _height(height), _width(width), _rotatable(rotatable), _transparent_piece(0, 2, 2, 2, 2), _is_solved(false)
{
	int i, j;
	for (i = 0; i < height; i++) {
...
// GETTERS
bool R_Solution::is_solved() { return _is_solved; }
...
// SETTERS
void R_Solution::set_solved(bool bool_set) { _is_solved = bool_set; }
...
void R_Solution::algorithm_step(std::stack<R_Solution>* sol_stack_ptr) {
...
void R_Solution::set_min_piece_fit_position_indices(int* row, int* column) {
	...
	int i, j;
	int min_size_of_position_fit_pieces = (_height * _width * 2) + 1;
	bool slot_available;
